module Crunchy
  class Index
    ...
    # {Crunchy::Index::Adapter::Base#import_fields}, in case when the Object
    ...
    #
    # @see Crunchy::Index::Actions::ClassMethods#sync
    class Syncer
      ...
      OUTDATED_IDS_WORKER = lambda do |outdated_sync_field_type, source_data_hash, index, total, index_data|
        ::Process.setproctitle("crunchy [#{index}]: sync outdated calculation (#{::Parallel.worker_number + 1}/#{total})") if index
        index_data.each_with_object([]) do |(id, index_sync_value), result|
          ...
          outdated = if outdated_sync_field_type == 'date'
            !Crunchy::Index::Syncer.dates_equal(typecast_date(source_data_hash[id]), Time.iso8601(index_sync_value))
          else
      ...
      SOURCE_OR_INDEX_DATA_WORKER = lambda do |syncer, index, kind|
        ::Process.setproctitle("crunchy [#{index}]: sync fetching data (#{kind})")
        result = case kind
        when :source
      ...
      # @param index [Crunchy::Index] crunchy index
      ...
      #
      # @see Crunchy::Index::Mapping::ClassMethods#supports_outdated_sync?
