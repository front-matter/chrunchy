require 'crunchy/index/import/journal_builder'
require 'crunchy/index/import/bulk_builder'
require 'crunchy/index/import/bulk_request'
require 'crunchy/index/import/routine'
  ...
  class Index
    module Import
      ...
      IMPORT_WORKER = lambda do |index, options, total, ids, iteration|
        ::Process.setproctitle("crunchy [#{index}]: import data (#{iteration + 1}/#{total})")
      ...
      LEFTOVERS_WORKER = lambda do |index, options, total, body, iteration|
        ::Process.setproctitle("crunchy [#{index}]: import leftovers (#{iteration + 1}/#{total})")
        routine = Routine.new(index, **options)
      ...
      module ClassMethods
        ...
        # objects later by listening to the `import_objects.crunchy` queue. It is also
        ...
        #
        # @raise [Crunchy::ImportFailed] in case of errors
        ruby2_keywords def import!(*args)
          errors = intercept_import_using_strategy(*args)

          raise Crunchy::ImportFailed.new(self, errors) if errors.present?
        ...
        #
        # @see https://github.com/elastic/elasticsearch-ruby/blob/master/elasticsearch-api/lib/elasticsearch/api/actions/bulk.rb
        # @see Crunchy::Index::Import::BulkRequest
        # @param options [Hash{Symbol => Object}] besides specific import options, it accepts all the options suitable for the bulk API call like `refresh` or `timeout`
        ...
        def compose(object, crutches = nil, fields: [])
          crutches ||= Crunchy::Index::Crutch::Crutches.new self, [object]
      ...
      private
        ...
        def intercept_import_using_strategy(*args)
          ...
          when :delayed_sidekiq
            begin
              Crunchy::Strategy::DelayedSidekiq::Scheduler.new(self, ids, options).postpone
        ...
        def import_linear(objects, routine)
          ActiveSupport::Notifications.instrument 'import_objects.crunchy', index: self do |payload|
