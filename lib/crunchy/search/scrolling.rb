  ...
  module Search
    ...
    module Scrolling
      ...
      def scroll_batches(batch_size: Request::DEFAULT_BATCH_SIZE, scroll: Request::DEFAULT_SCROLL)
        ...
        Crunchy.client.clear_scroll(body: {scroll_id: scroll_id}) if scroll_id
      end
      ...
      # each hit wrapped with {Crunchy::Index}.
      #
      # @param batch_size [Integer] batch size obviously, replaces `size` query parameter
      # @param scroll [String] cursor expiration time
      #
      ...
      #   @yieldparam object [Crunchy::Index] block is executed for each hit object
      #
      # @overload scroll_wrappers(batch_size: 1000, scroll: '1m')
      #   @example
      ...
      # Uses {Crunchy::Search::Request#load} passed options for loading.
      #
    ...
    private
      ...
      def perform_scroll(body)
        ActiveSupport::Notifications.instrument 'search_query.crunchy', notification_payload(request: body) do
          Crunchy.client.scroll(body)
        end
      end
    end
  end
