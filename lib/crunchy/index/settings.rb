module Crunchy
  class Index
    ...
    #
    # @example
    #   Crunchy.analyzer :title_analyzer, type: 'custom', filter: %w(lowercase icu_folding title_nysiis)
    #   Crunchy.filter :title_nysiis, type: 'phonetic', encoder: 'nysiis', replace: false
    #
    # `title_nysiis` filter here will be expanded automatically when
    # `title_analyzer` analyser will be used in index settings:
    #
    # @example
    #   class ProductsIndex < Crunchy::Index
    #     settings analysis: {
    #       analyzer: [
    #         'title_analyzer',
    ...
    class Settings
      ...
      def to_hash
        ...
        if settings[:index] || Crunchy.configuration[:index]
          settings[:index] = (Crunchy.configuration[:index] || {})
    ...
    private
      ...
      def resolve_analysis(analysis)
        analyzer = resolve(analysis[:analyzer], Crunchy.analyzers)

        options = %i[tokenizer filter char_filter].each.with_object({}) do |type, result|
          dependencies = collect_dependencies(type, analyzer)
          resolved = resolve(dependencies.push(analysis[type]), Crunchy.send(type.to_s.pluralize))
