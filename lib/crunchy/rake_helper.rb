module Crunchy
  module RakeHelper
    ...
    class << self
      ...
      #
      # @example
      #   Crunchy::RakeHelper.reset # resets everything
      #   Crunchy::RakeHelper.reset(only: 'cities') # resets only CitiesIndex
      #   Crunchy::RakeHelper.reset(only: ['cities', CountriesIndex]) # resets CitiesIndex and CountriesIndex
      #   Crunchy::RakeHelper.reset(except: CitiesIndex) # resets everything, but CitiesIndex
      #   Crunchy::RakeHelper.reset(only: ['cities', 'countries'], except: CitiesIndex) # resets only CountriesIndex
      #
      # @param only [Array<Crunchy::Index, String>, Crunchy::Index, String] index or indexes to reset; if nothing is passed - uses all the indexes defined in the app
      # @param except [Array<Crunchy::Index, String>, Crunchy::Index, String] index or indexes to exclude from processing
      # @param parallel [true, Integer, Hash] any acceptable parallel options for import
      # @param output [IO] output io for logging
      # @return [Array<Crunchy::Index>] indexes that were reset
      ...
      #
      # @example
      #   Crunchy::RakeHelper.upgrade # resets everything
      #   Crunchy::RakeHelper.upgrade(only: 'cities') # resets only CitiesIndex
      #   Crunchy::RakeHelper.upgrade(only: ['cities', CountriesIndex]) # resets CitiesIndex and CountriesIndex
      #   Crunchy::RakeHelper.upgrade(except: CitiesIndex) # resets everything, but CitiesIndex
      #   Crunchy::RakeHelper.upgrade(only: ['cities', 'countries'], except: CitiesIndex) # resets only CountriesIndex
      #
      # @param only [Array<Crunchy::Index, String>, Crunchy::Index, String] index or indexes to reset; if nothing is passed - uses all the indexes defined in the app
      # @param except [Array<Crunchy::Index, String>, Crunchy::Index, String] index or indexes to exclude from processing
      # @param parallel [true, Integer, Hash] any acceptable parallel options for import
      # @param output [IO] output io for logging
      # @return [Array<Crunchy::Index>] indexes that were actually reset
      ...
      #
      # @example
      #   Crunchy::RakeHelper.update # updates everything
      #   Crunchy::RakeHelper.update(only: 'places') # updates only PlacesIndex
      #   Crunchy::RakeHelper.update(except: PlacesIndex) # updates everything, but PlacesIndex
      #
      # @param only [Array<Crunchy::Index, String>, Crunchy::Index, String] indexes to update; if nothing is passed - uses all the indexes defined in the app
      # @param except [Array<Crunchy::Index, String>, Crunchy::Index, String] indexes to exclude from processing
      # @param parallel [true, Integer, Hash] any acceptable parallel options for import
      # @param output [IO] output io for logging
      # @return [Array<Crunchy::Index>] indexes that were actually updated
        ...
        subscribed_task_stats(output) do
          indexes_from(only: only, except: except).each_with_object([]) do |index, updated_indexes|
            if index.exists?
              ...
              output.puts "Skipping #{index}, it does not exists (use rake crunchy:reset[#{index.derivable_name}] to create and update it)"
            end
          end
        end
      end
      ...
      #
      # @example
      #   Crunchy::RakeHelper.sync # synchronizes everything
      #   Crunchy::RakeHelper.sync(only: 'places') # synchronizes only PlacesIndex
      #   Crunchy::RakeHelper.sync(except: PlacesIndex) # synchronizes everything, but PlacesIndex
      #
      # @param only [Array<Crunchy::Index, String>, Crunchy::Index, String] indexes to synchronize; if nothing is passed - uses all the indexes defined in the app
      # @param except [Array<Crunchy::Index, String>, Crunchy::Index, String] indexes to exclude from processing
      ...
      #
      # @example
      #   Crunchy::RakeHelper.journal_apply(time: 1.minute.ago) # applies entries created for the last minute
      #   Crunchy::RakeHelper.journal_apply(time: 1.minute.ago, only: 'places') # applies only PlacesIndex entries created for the last minute
      #   Crunchy::RakeHelper.journal_apply(time: 1.minute.ago, except: PlacesIndex) # applies everything, but PlacesIndex, entries created for the last minute
      #
      ...
      def journal_apply(time: nil, only: nil, except: nil, output: $stdout)
        ...
        subscribed_task_stats(output) do
          ...
          count = Crunchy::Journal.new(journal_indexes_from(only: only, except: except)).apply(time)
      ...
      #
      # @example
      #   Crunchy::RakeHelper.journal_clean # cleans everything
      #   Crunchy::RakeHelper.journal_clean(time: 1.minute.ago) # leaves only entries created for the last minute
      #   Crunchy::RakeHelper.journal_clean(only: 'places') # cleans only PlacesIndex entries
      #   Crunchy::RakeHelper.journal_clean(except: PlacesIndex) # cleans everything, but PlacesIndex entries
      #
      ...
      def journal_clean(time: nil, only: nil, except: nil, delete_by_query_options: {}, output: $stdout)
        subscribed_task_stats(output) do
          ...
          response = Crunchy::Journal.new(journal_indexes_from(only: only, except: except)).clean(time, delete_by_query_options: delete_by_query_options)
      ...
      #
      # @example
      #   Crunchy::RakeHelper.journal_create # creates journal
      #
      # @param output [IO] output io for logging
      # @return Crunchy::Index Returns instance of crunchy index
      def journal_create(output: $stdout)
        subscribed_task_stats(output) do
          Crunchy::Stash::Journal.create!
        end
      end

      # Eager loads and returns all the indexes defined in the application
      # except Crunchy::Stash::Specification and Crunchy::Stash::Journal.
      #
      # @return [Array<Crunchy::Index>] indexes found
      def all_indexes
        Crunchy.eager_load!
        Crunchy::Index.descendants - [Crunchy::Stash::Journal, Crunchy::Stash::Specification]
      end

      # Reindex data from source index to destination index
      #
      # @example
      #   Crunchy::RakeHelper.reindex(source: 'users_index', dest: 'cities_index') reindex data from 'users_index' index to 'cities_index'
      #
      ...
      def reindex(source:, dest:, output: $stdout)
        subscribed_task_stats(output) do
          ...
          Crunchy::Index.reindex(source: source, dest: dest)
      ...
      #
      # @example
      #   Crunchy::RakeHelper.update_mapping('cities', {properties: {new_field: {type: :text}}}) update 'cities' index with new_field of text type
      #
      ...
      #
      # @example
      #   Crunchy::RakeHelper.delete_by_query_options_from_env({'WAIT_FOR_COMPLETION' => 'false','REQUESTS_PER_SECOND' => '10','SCROLL_SIZE' => '5000'})
      ...
      def normalize_index(identifier)
        return identifier if identifier.is_a?(Class) && identifier < Crunchy::Index
      ...
      def subscribed_task_stats(output = $stdout, &block)
        start = Time.now
        ActiveSupport::Notifications.subscribed(JOURNAL_CALLBACK.curry[output], 'apply_journal.crunchy') do
          ActiveSupport::Notifications.subscribed(IMPORT_CALLBACK.curry[output], 'import_objects.crunchy', &block)
        end
      ensure
    ...
    private
      ...
      def warn_missing_index(output)
        ...
        output.puts "############################################################\n" \
                    ...
                    "      See https://github.com/toptal/crunchy#journaling\n" \
                    '############################################################'
      end

      def journal_exists?
        @journal_exists = Crunchy::Stash::Journal.exists? if @journal_exists.nil?
