...
# To use it - add `require 'crunchy/rspec'` to the `spec_helper.rb`
...
RSpec::Matchers.define :update_index do |index_name, options = {}| # rubocop:disable Metrics/BlockLength
  ...
  match do |block| # rubocop:disable Metrics/BlockLength
    ...
    index = crunchy.derive_name(index_name)
    if defined?(Mocha) && RSpec.configuration.mock_framework.to_s == 'RSpec::Core::MockingAdapters::Mocha'
      ...
      crunchy::Index::Import::BulkRequest.stubs(:new).with(index, params_matcher).returns(mock_bulk_request)
    else
      mocked_already = ::RSpec::Mocks.space.proxy_for(crunchy::Index::Import::BulkRequest).method_double_if_exists_for_message(:new)
      allow(crunchy::Index::Import::BulkRequest).to receive(:new).and_call_original unless mocked_already
      params_matcher = @no_refresh ? hash_including(refresh: false) : any_args
      allow(crunchy::Index::Import::BulkRequest).to receive(:new).with(index, params_matcher).and_return(mock_bulk_request)
    end

    crunchy.strategy(options[:strategy] || :atomic) { block.call }
