module Crunchy
  ...
  #
  # @example
  #   journal = Crunchy::Journal.new('places', UsersIndex)
  #   journal.apply(20.minutes.ago)
  #   journal.clean
  #
  class Journal
    # @param only [Array<String, Crunchy::Index>] indexes or string references to perform actions on
    def initialize(*only)
      @only = only
    end
    ...
    def apply(since_time, fetch_limit: 10, **import_options)
      ...
      while count < total_count
        ...
        ActiveSupport::Notifications.instrument 'apply_journal.crunchy', stage: stage, groups: groups
  ...
  private
    ...
    def entries(since_time, fetch_limit)
      Crunchy::Stash::Journal.entries(since_time, only: @only).order(:created_at).limit(fetch_limit)
    end

    def reference_groups(entries)
      entries.group_by(&:index_name)
        .transform_keys { |index_name| Crunchy.derive_name(index_name) }
