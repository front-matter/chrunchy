    ...
    module Actions
      ...
      module ClassMethods
        ...
        def create!(suffix = nil, **options)
          ...
          Crunchy.wait_for_status if result
          result
        end
        ...
        def reset!(suffix = nil, apply_journal: true, journal: false, **import_options)
          result = if suffix.present?
            ...
            result = import(**import_options.merge(
              suffix: suffix,
              journal: journal,
              refresh: !Crunchy.reset_disable_refresh_interval
            ))
        ...
        # A {Crunchy::Journal} instance for the particular index
        #
        # @return [Crunchy::Journal] journal instance
        def journal
          @journal ||= Crunchy::Journal.new(self)
        end
        ...
        #
        # @example
        #   Crunchy.client.update_mapping('cities', {properties: {new_field: {type: :text}}})
        #
      ...
      private
        ...
        def optimize_index_settings(index_name)
          settings = {}
          settings[:refresh_interval] = -1 if Crunchy.reset_disable_refresh_interval
          settings[:number_of_replicas] = 0 if Crunchy.reset_no_replicas
          update_settings index_name, settings: settings if settings.any?
        end

        def original_index_settings(index_name)
          settings = {}
          if Crunchy.reset_disable_refresh_interval
            settings.merge! index_settings(:refresh_interval)
            settings[:refresh_interval] = '1s' if settings.empty?
          end
          settings.merge! index_settings(:number_of_replicas) if Crunchy.reset_no_replicas
          update_settings index_name, settings: settings if settings.any?
        end
